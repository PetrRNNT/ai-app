...
const _sfc_main$1 = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      _push(`<div${ssrRenderAttrs(mergeProps({ class: "space-y-4" }, _attrs))}><div class="flex justify-between items-center"><h3 class="text-lg font-semibold text-gray-900">Шаблоны</h3><button class="btn-primary text-sm"> + Новый шаблон </button></div>`);
      if (unref(loading)) {
      ...
      if (unref(showCreate)) {
        _push(`<div class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50"><div class="bg-white rounded-lg p-6 w-full max-w-md"><h3 class="text-lg font-semibold mb-4">Новый шаблон</h3><form><input${ssrRenderAttr("value", unref(newTemplate).name)} type="text" placeholder="Название" class="input-base mb-4" required><select class="input-base mb-4"><option value="TASK"${ssrIncludeBooleanAttr(Array.isArray(unref(newTemplate).type) ? ssrLooseContain(unref(newTemplate).type, "TASK") : ssrLooseEqual(unref(newTemplate).type, "TASK")) ? " selected" : ""}>Шаблон задачи</option><option value="PROJECT"${ssrIncludeBooleanAttr(Array.isArray(unref(newTemplate).type) ? ssrLooseContain(unref(newTemplate).type, "PROJECT") : ssrLooseEqual(unref(newTemplate).type, "PROJECT")) ? " selected" : ""}>Шаблон проекта</option></select><textarea placeholder="Описание" class="input-base mb-4" rows="3">${ssrInterpolate(unref(newTemplate).description)}</textarea><label class="flex items-center gap-2 mb-4"><input${ssrIncludeBooleanAttr(Array.isArray(unref(newTemplate).isPublic) ? ssrLooseContain(unref(newTemplate).isPublic, null) : unref(newTemplate).isPublic) ? " checked" : ""} type="checkbox" class="rounded"><span class="text-sm text-gray-700">Публичный шаблон</span></label><div class="flex justify-end gap-2"><button type="button" class="btn-secondary">Отмена</button><button type="submit" class="btn-primary">Создать</button></div></form></div></div>`);
      } else {
        _push(`<!---->`);
      }
      _push(`</div>`);
...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`<h1 class="text-xl font-bold text-gray-900">Шаблоны</h1></div><div class="flex items-center space-x-4">`);
      if (unref(user)) {
...
//# sourceMappingURL=index-DGlEk0gG.mjs.map
